                                                    ...
                                                    {!isSelected ? (
                                                        (() => {
                                                            ...
                                                            if (selectedSystem?.id === 'x3n-ai-premium') {
                                                                ...
                                                                // Check X3N AHD camera limit
                                                                const x3nAHDCameraIds = ['ahd-outdoor-camera', 'ca46-blind-spot-camera', '720p-ahd-outdoor-camera', 'ca20s-adas-camera'];
                                                                if (x3nAHDCameraIds.includes(accessory.id)) {
                                                                    const currentAHDCount = getX3NAHDCameraCount();
                                                                    if (currentAHDCount >= 4) {
                                                                        x3nRestriction = 'AHD Cameras: Max 4 Total';
                                                                    }
                                                                }

                                                                // Check X3N IPC camera limit
                                                                const x3nIPCCameraIds = ['1080p-ipc-waterproof-camera', 'dsm-camera-kit'];
                                                                if (x3nIPCCameraIds.includes(accessory.id)) {
                                                                    const currentIPCCount = getX3NIPCCameraCount();
                                                                    if (currentIPCCount >= 4) {
                                                                        x3nRestriction = 'IP Cameras: Max 4 Total';
                                                                    }
                                                                }
                                                            }
                                        ...
                                        {selectedAccessories.map((acc) => (
                                            <div key={acc.product.id} className="flex items-center justify-between text-sm">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <button
                                                        onClick={() => handleUpdateQuantity(acc.product.id, 1)}
                                                        disabled={(selectedSystem?.id === 'c6d-ai-basic' && acc.quantity >= 2) || acc.product.id === 'camera-extended-cable' || ['ca29p-dms-camera', 'c29n-dms-camera'].includes(acc.product.id) || (acc.product.id === 'cp7-display' && selectedSystem?.id === 'ad-plus-advanced') || (['ahd-outdoor-camera', '720p-ahd-outdoor-camera'].includes(acc.product.id) && selectedSystem?.id === 'ad-plus-advanced') || (acc.product.id === '1080p-ipc-waterproof-camera' && selectedSystem?.id === 'f6n-mobile-dvr') || (['wd-blue-500gb-hdd', 'wd-scorpio-blue-1tb'].includes(acc.product.id) && selectedSystem?.id === 'x3n-ai-premium') || (['ca29p-dms-camera', 'ahd-outdoor-camera', 'ca46-blind-spot-camera', '720p-ahd-outdoor-camera'].includes(acc.product.id) && selectedSystem?.id === 'f6n-mobile-dvr' && getF6NAHDCameraCount() >= 4) || (['kingston-512gb-microsd', 'kingston-256gb-microsd', 'kingston-128gb-microsd'].includes(acc.product.id) && ['fs-c6-lite-standard', 'c6d-ai-basic', 'ad-plus-advanced', 'f6n-mobile-dvr'].includes(selectedSystem?.id || '') && getTotalMicroSDCount() >= 2) || (acc.product.id === 'dsm-camera-kit' && selectedSystem?.id === 'x3n-ai-premium') || (acc.product.id === 'cp4-display-kit' && selectedSystem?.id === 'x3n-ai-premium') || (acc.product.id === 'ca20s-adas-camera' && selectedSystem?.id === 'x3n-ai-premium') || (['ahd-outdoor-camera', 'ca46-blind-spot-camera', '720p-ahd-outdoor-camera', 'ca20s-adas-camera'].includes(acc.product.id) && selectedSystem?.id === 'x3n-ai-premium' && getX3NAHDCameraCount() >= 4) || (['1080p-ipc-waterproof-camera', 'dsm-camera-kit'].includes(acc.product.id) && selectedSystem?.id === 'x3n-ai-premium' && getX3NIPCCameraCount() >= 4)}
                                                        className="p-1 hover:bg-dark-700 rounded transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                                                    >
                                                        <Plus size={14} />
                                                    </button>
